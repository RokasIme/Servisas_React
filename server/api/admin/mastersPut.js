...
export async function mastersPut(req, res) {
  ...
  try {
    ...
    const sql = `UPDATE masters SET ${sqlColumns.map((s) => s + " = ?").join(", ")} WHERE id = ?;`;
    const [result] = await connection.execute(sql, [...sqValues, +req.params.id]);

    if (result.affectedRows !== 1) {
      return res.status(500).json({
        status: "error",
        msg: "Serverio klaida, pabandykite meistrą atnaujinti veliau2",
      });
    }
  } catch (error) {
    console.log(error);
    return res.status(500).json({
      status: "error",
      msg: "Serverio klaida, pabandykite meistrą atnaujinti veliau3",
    });
  }

  return res.json({
    status: "success",
    msg: "Meistras atnaujintas sekmingai",
  });
}
