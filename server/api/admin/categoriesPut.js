...
export async function categoriesPut(req, res) {
  ...
  try {
    const sql = "SELECT * FROM categories WHERE (category = ? OR url_slug = ?) AND id != ?;";
    const [result] = await connection.execute(sql, [name, url, +req.params.id]);

    if (result.length > 0) {
      return res.json({
        status: "error",
        msg: "Tokia meistrų kategorija jau egzistuoja.",
      });
    }
  } catch (error) {
    console.log(error);
  ...
  try {
    const sql = `
            UPDATE categories
            SET category = ?, url_slug = ?, description = ?, is_published = ?
            WHERE id = ?;`;
  ...
  return res.json({
    status: "success",
    msg: "Meistrų kategorija atnaujinta sekmingai",
  });
}
